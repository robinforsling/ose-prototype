...
are defined here: $\boldsymbol{x}$, $\boldsymbol{u}$, $\boldsymbol{\theta}$,
$\boldsymbol{\lambda}$, $\boldsymbol{\eta}$, $\mathcal{U}$, $\mathcal{X}$,
$\mathcal{S}_q$, and the rest. A page under `docs/models/` uses the same
symbols so the two read together, and a field name in the code maps to one of
them.
...
Both have happened. The switching set $\mathcal{S}_q$ carried a formulation
...
when no $\boldsymbol{\lambda}$ declared one. In both cases the document was
